...
public class UnitService {
  ...
  public List<Unit> getAllUnits() {
    return dbSupport.getJdbi()
      .withExtension(UnitDao.class, UnitDao::getAllUnits);
  }

  public List<Unit> getUnits(String providerId) {
    return dbSupport.getJdbi()
      .withExtension(UnitDao.class, dao -> dao.getUnits(providerId));
  }

  public Map<String, List<Unit>> getUnitsMultiple(List<String> providerIds) {
    return dbSupport.getJdbi()
      .withExtension(UnitDao.class, dao -> {
        var list = dao.getUnitsMultiple(providerIds);

        return list.stream().collect(
          Collectors.groupingBy(Unit::getProviderId, Collectors.toList()));
      });
  }

  public List<List<Unit>> getDerivedUnitsMultiple(List<Unit> units) {
    var tuples = units.stream()
      .map(e -> String.format("('%s', '%s')", e.providerId, e.unitName))
      .toList();
  ...
  public Unit createUnit(Unit pl) {
    return dbSupport.getJdbi()
      .withExtension(UnitDao.class,
        dao -> dao.insertUnit(pl)
      );
  }
  ...
  private Unit doUpdate(Unit pl) {
    var uDef = DbUtl.updateDef("units", pl)
      .where("providerId")
      .where("unitName")
  ...
  public Unit deleteUnit(Unit pl) {
    return dbSupport.getJdbi()
      .withExtension(UnitDao.class,
        dao -> dao.deleteUnit(pl.providerId, pl.unitName)
      );
  }
}
