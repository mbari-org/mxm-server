...
public class MissionTemplateAssetClassService {
  ...
  public List<MissionTemplateAssetClass> getMissionTemplateAssetClasses(String providerId) {
    return dbSupport.getJdbi()
      .withExtension(MissionTemplateAssetClassDao.class, dao -> dao.getMissionTemplateAssetClasses(providerId));
  }

  public List<String> getAssetClassNames(String providerId, String missionTplId) {
    return dbSupport.getJdbi()
      .withExtension(MissionTemplateAssetClassDao.class, dao -> dao.getAssetClassNames(providerId, missionTplId));
  }
  ...
  public class AssetClassWithMissionTplId extends AssetClass {
    public String missionTplId;
  }

  private static final RowMapper<AssetClassWithMissionTplId> assetClassWithMissionTplIdMapper = new RowMapper<AssetClassWithMissionTplId>() {
    @Override
    public AssetClassWithMissionTplId map(ResultSet rs, StatementContext ctx) throws SQLException {
  ...
  public List<List<AssetClass>> getAssetNamesMultiple(List<MissionTemplate> missionTemplates) {
    ...
    return tuples.stream().map(byTupleId::get)
      .map(list -> list == null ? null : list.stream()
        .map(ac -> (AssetClass) ac)
        .collect(Collectors.toList())
      )
      .collect(Collectors.toList());
  }
